...
contract DummyERC20 is ERC20, Pausable, Ownable {
    ...
    function burnFrom(address account, uint256 amount) public {
        uint256 decreasedAllowance = allowance(account, _msgSender()).sub(amount, "DUMMY: burn amount exceeds allowance");
    ...
    function release(address account, uint256 releaseAmount) public onlyOwner {
        require(account != address(0), "DUMMY: release zero address");
    ...
    function transferWithLock(address recipient, uint256 amount, uint256 releaseTime) public onlyOwner returns (bool) {
        require(recipient != address(0), "DUMMY: lockup zero address");
        require(releaseTime > block.timestamp, "DUMMY: release time before lock time");
        require(_timelock[recipient].releaseTime == 0, "DUMMY: already locked");
    ...
    function batchTransfer(address[] memory recipients, uint256[] memory amounts) public {
        require(recipients.length > 0, "DUMMY: least one recipient address");
        require(recipients.length == amounts.length, "DUMMY: number of recipient addresses does not match the number of tokens");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        require(!paused(), "DUMMY: token transfer while paused");
        ...
        if(timelock.releaseTime != 0 && balanceOf(from).sub(amount) < timelock.amount) {
            require(block.timestamp >= timelock.releaseTime, "DUMMY: current time is before from account release time");
